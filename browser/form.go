...
func (f *Form) Input(name, value string) error {
	...
	return mzgserrors.NewElementNotFound("No input found with name '%s'.", name)
}
...
func (f *Form) File(name string, fileName string, data io.Reader) error {
	...
	return mzgserrors.NewElementNotFound(
...
func (f *Form) Check(name string) error {
	...
	return mzgserrors.NewElementNotFound("No checkbox found with name '%s'.", name)
}
...
func (f *Form) IsChecked(name string) (bool, error) {
	...
	return false, mzgserrors.NewElementNotFound("No checkbox found with name '%s'.", name)
}
...
func (f *Form) SelectByOptionLabel(name string, optionLabel ...string) error {
	s, ok := f.selects[name]
	if !ok {
		return mzgserrors.NewElementNotFound("No select element found with name '%s'.", name)
	}
	if len(optionLabel) > 1 && !s.multiple {
		return mzgserrors.NewElementNotFound("The select element with name '%s' is not a select miltiple.", name)
	}
	f.fields.Del(name)
	for _, l := range optionLabel {
		if _, ok := s.labels[l]; !ok {
			return mzgserrors.NewElementNotFound("The select element with name %q does not have an option with label %q", name, l)
		}
...
func (f *Form) SelectByOptionValue(name string, optionValue ...string) error {
	...
	for _, v := range optionValue {
		if _, ok := s.values[v]; !ok {
			return mzgserrors.NewElementNotFound("The select element with name %q does not have an option with value %q", name, v)
		}
		f.fields.Add(name, v)
	}
	return nil
...
func (f *Form) SelectValues(name string) ([]string, error) {
	...
	return nil, mzgserrors.NewElementNotFound("No input found with name '%s'.", name)
}
...
func (f *Form) SelectLabels(name string) ([]string, error) {
	s, ok := f.selects[name]
	if !ok {
		return nil, mzgserrors.NewElementNotFound("No select element found with name '%s'.", name)
	}
	var labels []string
	for _, v := range f.fields[name] {
...
func (f *Form) Click(button string) error {
	if _, ok := f.buttons[button]; !ok {
		return mzgserrors.NewInvalidFormValue(
