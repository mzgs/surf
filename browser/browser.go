...
func (bow *Browser) Reload() error {
	...
	return mzgserrors.NewPageNotLoaded("Cannot reload, the previous request failed.")
}
...
func (bow *Browser) Form(expr string) (Submittable, error) {
	sel := bow.Find(expr)
	if sel.Length() == 0 {
		return nil, mzgserrors.NewElementNotFound(
...
func (bow *Browser) attrToResolvedUrl(name string, sel *goquery.Selection) (*url.URL, error) {
	...
	if !ok {
		return nil, mzgserrors.NewAttributeNotFound(
